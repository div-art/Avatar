...
class Avatar
{
    ...
    public function createShape($fileName)
    {
        ...
        $bbox = imagettfbbox($this->fontSize, 0, public_path("fonts/$this->fontFamily.ttf"), $this->userInitials);
        $bbox[0] = 0;
        $x = ($this->width / 2) - (($bbox[2] - $bbox[0]) / 2);

        $y = ($this->height / 2) - (($bbox[7] - $bbox[1]) / 2);

        imagettftext($im, $this->fontSize, 0, $x, $y, imagecolorallocate($im, $red, $green, $blue), public_path("fonts/$this->fontFamily.ttf"), $this->userInitials);
